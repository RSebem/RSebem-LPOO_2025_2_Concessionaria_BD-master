        ...
        tblVeiculos.getModel().addTableModelListener(e -> {
            ...
            if (col == 4) {
                ...
                dao.buscarPorPlaca(placa).ifPresent(v -> {
                    int op_edit = JOptionPane.showConfirmDialog(rootPane, "Tem certeza que deseja alterar a disponibilidade?");
                    if (op_edit == JOptionPane.YES_OPTION) {
                        Boolean disponivel = (Boolean) tblVeiculos.getValueAt(row, 4);
                        v.setDisponivel(disponivel);
                        try {
                            dao.persist(v);
                        } catch (Exception ex) {
                            System.out.println("Erro ao alterar a disponibilidade do veiculo: " + v + "\nErro: " + ex);
                        }
                    }
                });
            }
